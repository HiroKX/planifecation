...
export default function App() {
    ...
    const menuHeight = 20; // Hauteur du menu en pixels
    const circleSize = 50; // Diamètre du cercle

    // Valeur de compensation pour l'axe Z lorsque le téléphone est à plat
    const zCompensation = -0.5;
    ...
    useEffect(() => {
        ...
        const subscription = DeviceMotion.addListener(({ accelerationIncludingGravity }) => {
            ...
            const boundedX = Math.max(-screenWidth / 2, Math.min(newX, screenWidth / 2 - circleSize));
            const boundedY = Math.max(-screenHeight / 2 + menuHeight, Math.min(newY, screenHeight / 2 - circleSize));
